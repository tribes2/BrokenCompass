...
function AIsensorObjectiveInit(%object)
{
   if(%object.team > 0)
   {
      ...
      addAIObjective(%enemyTeam, createDefaultMissile(%object, $AIWeightMortarTurret[1], $AIWeightMortarTurret[2]));
      addAIObjective(%enemyTeam, createDefaultAttack(%object, $AIWeightMortarTurret[1], $AIWeightMortarTurret[2]));
   }
   else
   {
      ...
      addAIObjective(1, createDefaultMissile(%object, $AIWeightMortarTurret[1], $AIWeightMortarTurret[2]));
      addAIObjective(1, createDefaultAttack(%object, $AIWeightMortarTurret[1], $AIWeightMortarTurret[2]));
      ...
      addAIObjective(2, createDefaultMissile(%object, $AIWeightMortarTurret[1], $AIWeightMortarTurret[2]));
      addAIObjective(2, createDefaultAttack(%object, $AIWeightMortarTurret[1], $AIWeightMortarTurret[2]));
   }
}
...
//--------------------------------------------------------------------------------------------------------
//---------------------- New Default AIO Missile Object ---Lagg... - 2-25-2003 

function createDefaultMissile(%object, %weight1, %weight2)
{
   %objective = new AIObjective(AIOMissileObject) 
   {
      dataBlock = "AIObjectiveMarker";
      description = "Missile the " @ %object.getDataBlock().getName();
		targetObject = %object.getName();
		targetObjectId = %object;
		targetClientId = -1;
		weightLevel1 = %weight1;
      ...
      location = %object.getWorldBoxCenter();
		offense = true;
		equipment = "MissileLauncher MissileLauncherAmmo";
...
function createDefaultAttack(%object, %weight1, %weight2)
{
   %objective = new AIObjective(AIOAttackObject) 
   {
      ...
      location = %object.getWorldBoxCenter();
		offense = true;
		desiredEquipment = "AmmoPack Plasma PlasmaAmmo";
		buyEquipmentSet = "HeavyAmmoSet";
	};
...
function AIflagObjectiveInit(%flag)
{
   ...
   %newObjective = new AIObjective(AIODefendLocation)
   {
      ...
      defense = true;
		desiredEquipment = "EnergyPack ELFgun";		
                buyEquipmentSet = "HeavyEnergyDefault";
